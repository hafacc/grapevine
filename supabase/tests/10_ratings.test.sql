...
select throws_ok(
  $$insert into public.ratings (user_id, item_id, tag, value)
    values ((select auth.uid()), 'café bleu', '', -1)$$, '23505',
  'and the key refuses a second thumb on the thing');
select throws_ok(
  $$insert into public.ratings (user_id, item_id, tag, value)
    values ((select auth.uid()), 'café bleu', 'late night', 1)$$, '23505',
...
select throws_ok(
  $$insert into public.ratings (user_id, item_id, value)
    values ((select auth.uid()), 'Café Bleu', 1)$$, '23514', 'item: a capital');
select throws_ok(
  $$insert into public.ratings (user_id, item_id, value)
    values ((select auth.uid()), repeat('i', 129), 1)$$, '23514', 'item: 129 characters');
select throws_ok(
  $$insert into public.ratings (user_id, item_id, tag, value)
    values ((select auth.uid()), 'café bleu', 'Late Night', 1)$$, '23514', 'tag: a capital');
select throws_ok(
  $$insert into public.ratings (user_id, item_id, tag, value)
    values ((select auth.uid()), 'café bleu', repeat('t', 129), 1)$$, '23514',
  'tag: 129 characters');
-- Empty is what the item's own thumb is keyed by, so it cannot be null as well.
select throws_ok(
  $$insert into public.ratings (user_id, item_id, tag, value)
    values ((select auth.uid()), 'café bleu', null, 1)$$, '23502', 'tag: null');

select throws_ok(
  $$insert into public.ratings (user_id, item_id, value)
    values ((select auth.uid()), 'dune', 99)$$, '23514', 'a value that is not a thumb');
select throws_ok(
  $$insert into public.ratings (user_id, item_id, value)
    values ((select auth.uid()), 'dune', 0)$$, '23514', 'nor is nought');

select throws_ok(
  $$insert into public.ratings (user_id, item_id, value, weight)
    values ((select auth.uid()), 'dune', 1, 99)$$, '42703',
...
select throws_ok(
  $$insert into public.ratings (user_id, item_id, value, rated_at)
    values ((select auth.uid()), 'dune', 1, 'epoch')$$, '42501',
...
set local role authenticated;
select throws_ok(
  $$update public.ratings set item_id = 'dune' where item_id = 'café bleu'$$, '42501',
  'but not re-pointed at another thing');
select throws_ok(
  $$update public.ratings set tag = 'cheap' where item_id = 'café bleu'$$, '42501',
  'nor at another attribute');
select throws_ok(
  $$update public.ratings set rated_at = 'epoch' where item_id = 'café bleu'$$, '42501',
...
set local request.jwt.claims = '{"sub":"11111111-1111-1111-1111-111111111111","role":"authenticated"}';
select throws_ok(
  $$select changed_at from private.ratings_changed$$, '42501',
...
select throws_ok(
  $$insert into public.ratings (user_id, item_id, value)
    values ('11111111-1111-1111-1111-111111111111', 'dune', 1)$$, '42501',
