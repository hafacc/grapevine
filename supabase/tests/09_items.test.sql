...
select throws_ok($$select id from public.items$$, '42501', null,
  'a signed-out visitor cannot read the catalog');
select throws_ok($$insert into public.items (id, search_id) values ('x', 'x')$$, '42501', null,
  'nor add to it');
...
select throws_ok($$update public.items set search_id = 'chase support' where id = 'café bleu'$$,
  '42501', null, 'an item cannot be re-indexed');
...
select throws_ok(
  ...
  '42501', null, 'an item cannot be backdated');
...
select throws_ok(
  ...
  '42501', null, 'and nobody can name anyone but themselves as creator');
