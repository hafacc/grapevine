...
select throws_ok(
  ...
  '42501', 'nobody writes a suggestion, their owner included');
select throws_ok(
  $$update public.suggestions set rank = 2$$, '42501', 'nor edits one');
select throws_ok(
  $$delete from public.suggestions$$, '42501', 'nor removes one');
...
select throws_ok(
  ...
  '42501', 'nor plants themselves in it');
...
select throws_ok(
  ...
  '42703', 'a field no reader knows about cannot be written');
select throws_ok(
  $$update public.user_prefs set discoverable_by_taste = 'not a boolean'
    where user_id = (select auth.uid())$$,
  '22P02', 'and neither can a wrong type');
