...
# cut-down `auth` schema and, if pgTAP is not installed, for the six pgTAP functions the suites
# use — so a suite that passes here and fails under `supabase test db` leaned on a stand-in.
# Use it to develop a suite; believe CI.
