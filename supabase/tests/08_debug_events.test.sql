...
select throws_ok(
  $$select public.record_debug_event('k', 'd', (select auth.uid()))$$, '42883',
...
select throws_ok(
  $$select public.record_debug_event('listeners-lost', repeat('x', 2001))$$, '23514',
  'an oversized detail is refused');
select throws_ok(
  $$select public.record_debug_event(repeat('k', 41), 'd')$$, '23514',
...
select throws_ok($$select * from private.debug_events$$, '42501',
  'nobody reads one back, including its author');
select throws_ok($$update private.debug_events set kind = 'x'$$, '42501',
  'nobody edits one');
select throws_ok($$delete from private.debug_events$$, '42501',
  'nobody removes one');
select throws_ok($$insert into private.debug_events (kind, detail) values ('k', 'd')$$, '42501',
...
select throws_ok(
  $$select public.record_debug_event('listeners-lost', 'd')$$, '42501',
