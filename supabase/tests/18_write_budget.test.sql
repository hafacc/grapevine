...
select throws_ok(
  ...
  'PT429', 'the next thumb is refused with a stable code');
select throws_ok(
  $$insert into public.items (id, search_id) values ('one more', 'one more')$$,
  'PT429', 'and so is the next item');
...
select throws_ok(
  ...
  'PT429', 'and so is the next connect request');
select lives_ok(
...
select throws_ok(
  $$insert into public.connect_requests (from_id, to_id)
    values ((select auth.uid()), '22222222-2222-2222-2222-222222222222')$$,
  'PT429', 'and sending it again does not');
...
select throws_ok(
  $$select writes from private.write_budget$$, '42501',
