...
select throws_ok($$select * from private.params$$, '42501',
  'a signed-in account cannot read the estimate');
select throws_ok($$select kappa from private.params$$, '42501',
  'nor one column of it');
select throws_ok($$insert into private.params (id, kappa) values (true, 1)$$, '42501',
  'nor write it');
select throws_ok($$update private.params set kappa = 1$$, '42501',
  'nor move a number in it');
select throws_ok($$delete from private.params$$, '42501',
  'nor remove it');
...
select throws_ok(
  $$insert into private.params (id, kappa) values (true, 2)$$, '23505',
  'there is one row and the key says so');
select throws_ok(
  $$insert into private.params (id, kappa) values (false, 2)$$, '23514',
