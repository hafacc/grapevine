...
select throws_ok(
  ...
  '42501', null, 'and `since` is the server''s, not the writer''s');

select throws_ok(
  $$insert into public.friendships (user_id, friend_id)
    values ('33333333-3333-3333-3333-333333333333', (select auth.uid()))$$,
  '42501', null, 'nobody writes into somebody''s friend list without a request');

select throws_ok(
  $$select public.accept_connect_request('33333333-3333-3333-3333-333333333333')$$,
  '42501', null, 'and accepting an ask nobody made is refused');
...
select throws_ok(
  ...
  '42501', null, 'and cannot then accept their own ask on the stranger''s behalf');
...
select throws_ok(
  ...
  '23514', null, 'one half of an edge alone cannot stand');
...
select throws_ok(
  ...
  '23514', null, 'and leaving removes both or neither');
select lives_ok(
  $$delete from public.friendships
