...
//
// Bumping this evicts every earlier cache on activate — every earlier one of
// OURS. The site has an origin of its own, so today nothing else writes to this
// Cache Storage; the prefix is what keeps it that way if another app ever
// shares the origin.
const CACHE_PREFIX = "grapevine-";
...
worker.addEventListener("install", (event) => {
  ...
  // the app and losing signal before opening it again gives the browser's
  ...
  event.waitUntil(
    (async () => {
      for (const name of await caches.keys()) {
        if (name.startsWith(CACHE_PREFIX) && name !== VERSION) {
...
// Cache Storage and local storage share ONE per-origin quota, and a browser
// under pressure evicts the origin whole — so a shell cache that grows without
// bound can cost the user the cached feed that makes an offline open worth
// anything.
...
async function store(key: Request, response: Response): Promise<void> {
  ...
  // nothing. No check would catch it: one deploy makes about 32 entries, so
  // none gets near the cap.
...
worker.addEventListener("fetch", (event) => {
  ...
  if (IMMUTABLE.test(url.pathname)) {
    event.respondWith(
      (async () => {
        ...
        // among it — would put the cache bookkeeping on the critical path of
  ...
  if (request.mode === "navigate") {
    ...
    // and the fragment, so caching the request as it comes writes whatever a
    // link happened to carry into a store with no expiry that any same-origin
    // script can read. The document is the same either way, so one entry per
    ...
    event.respondWith(
      (async () => {
        try {
          ...
          // catch-all: the written pages are their own routes, and serving Home
          // under one of their URLs would show the signed-in app instead of the
