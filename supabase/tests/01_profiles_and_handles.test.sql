...
select throws_ok(
  $$select public.claim_username('owner_h')$$, '23505',
  'a second claim by the same account raises rather than passing');

select throws_ok(
  $$select public.claim_username('findable_one')$$, '23505',
...
select throws_ok(
  $$update public.profiles set username = null where id = (select auth.uid())$$, '42501',
  'a handle cannot be released');
select throws_ok(
  $$update public.profiles set username = 'something_else' where id = (select auth.uid())$$, '42501',
  'nor changed, not even to a free one');
select throws_ok(
  $$update public.profiles set username = 'findable_one' where id = (select auth.uid())$$, '42501',
...
select throws_ok($$select public.claim_username('admin')$$,     '23514', 'reserved: admin');
select throws_ok($$select public.claim_username('grapevine')$$, '23514', 'reserved: grapevine');
select throws_ok($$select public.claim_username('support')$$,   '23514', 'reserved: support');
select throws_ok($$select public.claim_username('help')$$,      '23514', 'reserved: help');
select throws_ok($$select public.claim_username('root')$$,      '23514', 'reserved: root');
select throws_ok($$select public.claim_username('system')$$,    '23514', 'reserved: system');
select throws_ok($$select public.claim_username('about')$$,     '23514', 'reserved: about');
select throws_ok($$select public.claim_username('settings')$$,  '23514', 'reserved: settings');

select throws_ok($$select public.claim_username('ab')$$,       '23514', 'malformed: too short');
select throws_ok($$select public.claim_username('1abc')$$,     '23514', 'malformed: leading digit');
select throws_ok($$select public.claim_username('Bad_Caps')$$, '23514', 'malformed: capitals');
select throws_ok($$select public.claim_username('has-dash')$$, '23514', 'malformed: a dash, which the folding never emits');
...
-- and nowhere else.
select throws_ok(
  $$update public.profiles set email = 'owner@example.com' where id = (select auth.uid())$$, '42703',
  'there is no column to park an address in');
select throws_ok(
  $$update public.profiles set phone_number = '+15551234567' where id = (select auth.uid())$$, '42703',
  'nor a number');

-- A missing column privilege.
select throws_ok(
  $$update public.profiles set created_at = now() where id = (select auth.uid())$$, '42501',
  'created_at is not the client''s to move');

-- No INSERT and no DELETE verb at all: the row is the trigger's.
select throws_ok(
  $$insert into public.profiles (id, display_name) values (gen_random_uuid(), 'Nobody')$$, '42501',
  'a profile cannot be conjured');
select throws_ok(
  $$delete from public.profiles where id = (select auth.uid())$$, '42501',
  'nor deleted');
