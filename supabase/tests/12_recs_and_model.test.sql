...
select throws_ok(
  $$select count(*) from public.user_model$$, '42501',
...
select throws_ok(
  ...
  '42501', 'the owner cannot insert a feed');
select throws_ok(
  $$update public.user_recs set entries = '[]'::jsonb$$, '42501',
  'nor rewrite one');
select throws_ok(
  $$delete from public.user_recs$$, '42501',
  'nor delete one');
...
select throws_ok(
  ...
  '42501', 'the owner cannot insert a model row');
select throws_ok(
  $$update public.user_model set reach = '{"f1":1}'::jsonb$$, '42501',
  'nor rewrite one');
select throws_ok(
  $$delete from public.user_model$$, '42501',
  'nor delete one');
...
select throws_ok(
  ...
  '42501', 'nor can they plant one in it');
